    ...
    return (
        <div style={{ padding: "40px 48px" }}>
            <div style={{ display: "flex", justifyContent: "space-between", alignItems: "center", marginBottom: 40, borderBottom: "1px solid var(--color-rule)", paddingBottom: 20 }}>
                <div className="serif-heading" style={{ fontSize: 32, color: "var(--color-dark)" }}>Events Management</div>
                <button style={{ background: "var(--color-dark)", color: "var(--color-white)", padding: "10px 16px", border: "none", fontSize: 13, fontFamily: "var(--font-mono)", textTransform: "uppercase", letterSpacing: "0.1em", cursor: "pointer" }}>+ Create Event</button>
            </div>
                ...
                <table style={{ width: "100%", borderCollapse: "collapse", fontSize: 14, textAlign: "left" }}>
                    <thead>
                        <tr style={{ background: "var(--color-ivory-2)", fontFamily: "var(--font-mono)", fontSize: 11, color: "var(--color-dark)", letterSpacing: "0.1em", textTransform: "uppercase" }}>
                            <th style={{ padding: "16px 20px", borderBottom: "1px solid var(--color-rule)" }}>Event Title</th>
                            <th style={{ padding: "16px 20px", borderBottom: "1px solid var(--color-rule)" }}>Date & Time</th>
                    ...
                    <tbody>
                        {data.length === 0 ? (
                            <tr><td colSpan="5" style={{ padding: "32px", textAlign: "center", color: "var(--color-grey)" }}>No upcoming events.</td></tr>
                        ) : data.map(item => (
                            <tr key={item.id} style={{ borderBottom: "1px solid var(--color-rule-lt)" }}>
                                ...
                                <td style={{ padding: "16px 20px", color: "var(--color-grey)" }}>{new Date(item.date).toLocaleString()}</td>
                                <td style={{ padding: "16px 20px" }}>
                                    <span style={{ color: item.status === "Upcoming" ? "green" : "var(--color-grey)" }}>● {item.status}</span>
                                </td>
