    ...
    return (
        <div style={{ background: T.ivory, borderBottom: `1px solid ${T.ruleLt}`, padding: "18px 48px" }}>
            <div style={{ maxWidth: 1160, margin: "0 auto", display: "flex", alignItems: "center", gap: 28, flexWrap: "wrap" }}>
                <span style={{ fontFamily: F.mono, fontSize: 11, fontWeight: 700, color: T.grey, letterSpacing: "0.12em", textTransform: "uppercase", flexShrink: 0 }}>Endorsed & recognised by</span>
                ...
                {["AfDB", "UN-HABITAT", "AfCFTA Advisory Council", "FTID", "Former President Obasanjo"].map(l => (
                    <div key={l} style={{ fontFamily: F.mono, fontSize: 12, color: T.grey, letterSpacing: "0.08em", textTransform: "uppercase", fontWeight: 800 }}>{l}</div>
                ))}
            </div>
        </div>
    );
